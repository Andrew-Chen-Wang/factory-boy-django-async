...
from factory import enums, errors, utils
...
class BaseDeclaration(utils.OrderedBase):
    ...
    def _unroll_context(self, instance, step, context):
        full_context = dict()
        ...
        return full_context, subfactory

    def unroll_context(self, instance, step, context):
        context = self._unroll_context(instance, step, context)
        if isinstance(context, dict):
            return context
        full_context, subfactory = context
        return step.recurse(subfactory, full_context, force_sequence=step.sequence)

    async def aunroll_context(self, instance, step, context):
        context = self._unroll_context(instance, step, context)
        if isinstance(context, dict):
            return context
        full_context, subfactory = context
        return await step.arecurse(subfactory, full_context, force_sequence=step.sequence)
    ...
    async def aevaluate_pre(self, instance, step, overrides):
        context = await self.aunroll_context(instance, step, overrides)
        return await self.aevaluate(instance, step, context)
    ...
    async def aevaluate(self, instance, step, extra):
        return self.evaluate(instance, step, extra)
...
class LazyFunction(BaseDeclaration):
    ...
    async def aevaluate(self, instance, step, extra):
        logger.debug("LazyFunction: Evaluating %r on %r", self.function, step)
        return await self.function() if self.is_async else self.function()
...
class LazyAttribute(BaseDeclaration):
    ...
    async def aevaluate(self, instance, step, extra):
        logger.debug("LazyAttribute: Evaluating %r on %r", self.function, instance)
        return await self.function(instance) if self.is_async else self.function(instance)
...
class Sequence(BaseDeclaration):
    ...
    async def aevaluate(self, instance, step, extra):
        logger.debug("Sequence: Computing next value of %r for seq=%s", self.function, step.sequence)
        return await self.function(int(step.sequence)) if self.is_async else self.function(int(step.sequence))
...
class LazyAttributeSequence(Sequence):
    ...
    async def aevaluate(self, instance, step, extra):
        logger.debug(
            "LazyAttributeSequence: Computing next value of %r for seq=%s, obj=%r",
            self.function, step.sequence, instance)
        return await self.function(instance, int(step.sequence)) if self.is_async else self.function(instance, int(step.sequence))
...
class ContainerAttribute(BaseDeclaration):
    ...
    async def aevaluate(self, instance, step, extra):
        ...
        return await self.function(instance, chain) if self.is_async else self.function(instance, chain)
...
class ParameteredAttribute(BaseDeclaration):
    ...
    async def aevaluate(self, instance, step, extra):
        """See evaluate()"""
        return await self.agenerate(step, extra)
    ...
    async def agenerate(self, step, params):
        """See generate()"""
...
class SubFactory(BaseDeclaration):
    ...
    async def aevaluate(self, instance, step, extra):
        ...
        return await step.arecurse(subfactory, extra, force_sequence=force_sequence)
...
class PostGenerationDeclaration(BaseDeclaration):
    ...
    async def aevaluate_post(self, instance, step, overrides):
        context = await self.aunroll_context(instance, step, overrides)
        postgen_context = PostGenerationContext(
            value_provided=bool('' in context),
            value=context.get(''),
        ...
        return await self.acall(instance, step, postgen_context)
    ...
    async def acall(self, instance, step, context):  # pragma: no cover
        """See call()"""
        return self.call()
...
class PostGeneration(PostGenerationDeclaration):
    ...
    def _call(self, instance, step, context):
        logger.debug(
    ...
    def call(self, instance, step, context):
        return self._call(instance, step, context)

    async def acall(self, instance, step, context):
        if self.is_async:
            return await self._call(instance, step, context)
        else:
            return self._call(instance, step, context)
...
class RelatedFactory(PostGenerationDeclaration):
    ...
    def _call(self, instance, step, context):
        ...
        return factory, passed_kwargs

    def call(self, instance, step, context):
        factory, passed_kwargs = self._call(instance, step, context)
        return step.recurse(factory, passed_kwargs)

    async def acall(self, instance, step, context):
        factory, passed_kwargs = self._call(instance, step, context)
        return await step.arecurse(factory, passed_kwargs)
...
class RelatedFactoryList(RelatedFactory):
    ...
    async def acall(self, instance, step, context):
        ...
        return await asyncio.gather(*[
            parent.acall(instance, step, context)
            for i in range(self.size if isinstance(self.size, int) else self.size())
        ])
...
class PostGenerationMethodCall(PostGenerationDeclaration):
    ...
    def call(self, instance, step, context):
        args, kwargs = self._call(instance, step, context)
